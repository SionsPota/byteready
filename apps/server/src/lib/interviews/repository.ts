...
export const createInterviewRepository = (db: DatabaseSync) => {
  return {
    createSession: (input: CreateSessionInput): InterviewSessionRow => {
      ...
      const now = Date.now()
      db.prepare(
        'INSERT INTO training_sessions (id, owner_id, position, target_company, resume_id, status, created_at) VALUES (?, ?, ?, ?, ?, ?, ?)'
      ).run(id, input.ownerId, input.position, input.targetCompany ?? null, input.resumeId ?? null, 'pending', now)

      return {
        id, owner_id: input.ownerId, position: input.position,
    ...
    getById: (id: string): (InterviewSessionRow & { turns: TurnRow[] }) | null => {
      const row = db.prepare('SELECT * FROM training_sessions WHERE id = ?').get(id) as InterviewSessionRow | undefined
      if (!row) return null
      const turns = db.prepare('SELECT * FROM training_turns WHERE session_id = ? ORDER BY "index" ASC').all(id) as unknown as TurnRow[]
      return { ...row, turns }
    },
    ...
    createTurn: (input: CreateTurnInput): TurnRow => {
      ...
      const now = Date.now()
      db.prepare(
        'INSERT INTO training_turns (id, session_id, "index", kind, question_id, text, audio_meta, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?)'
